...
const calendarStyles = `
  ...
  .fc-event {
    ...
    background-color: #f59e0b; /* Cor laranja para eventos pessoais */
  }
  
  /* Cor e estilo específico para diferenciar dos Agendamentos */
...
export default function PersonalCalendarTab({ org }) {
  ...
  async function checkAuthorizationStatus() {
    setLoading(true);
    try {
      // Endpoint para verificar se o usuário já autorizou (e se o token é válido)
      const res = await fetch(
        `${API_BASE_URL}/api/admin/calendar/status/${slug}`,
      ...
      if (res.ok && data.isAuthorized) {
        ...
        // Se a resposta for 200/ok, mas isAuthorized for false, 
        // significa que a integração nunca foi feita ou foi revogada.
      }
    } catch (e) {
  ...
  async function loadGoogleEvents() {
    setLoading(true);
    try {
      // Endpoint que seu backend usa para buscar eventos do Google Calendar 
      // usando o token de acesso armazenado.
      const res = await fetch(
        `${API_BASE_URL}/api/admin/calendar/events/${slug}`,
        { credentials: "include", cache: "no-store" }
      );
      
      if (!res.ok) {
        // Se o token expirou e o refresh falhou, o backend deve retornar 401/403
        toast.warn("Token expirado. Por favor, reconecte o Google Calendar.");
        setIsAuthorized(false);
        setCalendarEvents([]);
        return;
      }
      ...
      const data = await res.json();
      // Mapeia os eventos para o formato esperado pelo FullCalendar
      const formattedEvents = data.map((event) => ({
        id: event.id,
        title: event.summary || "Evento Pessoal",
        start: event.start.dateTime || event.start.date,
        end: event.end.dateTime || event.end.date,
        // Adiciona uma classe para estilos personalizados
  ...
  // ===========================
  const handleConnect = () => {
    // Redireciona o usuário para o endpoint do seu backend que inicia o OAuth.
    // O backend será responsável por usar o CLIENT_ID e SECRET_KEY e 
    // redirecionar para a página de consentimento do Google.
    window.location.href = `${API_BASE_URL}/api/admin/calendar/connect/${slug}`;
  };

  // ===========================
  // 🔹 3. Componente de Autorização (Se não estiver autorizado)
  // ===========================
  const AuthPrompt = () => (
    <div className="flex flex-col items-center justify-center p-8 bg-gray-50 rounded-2xl shadow-xl border border-gray-200 dark:bg-gray-800 dark:border-gray-700 max-w-lg mx-auto mt-10 text-center">
      ...
      <p className="text-gray-600 dark:text-gray-300 mb-6 text-sm">
        Ao conectar sua agenda, garantimos que os horários marcados como "ocupado" 
        no seu calendário pessoal fiquem automaticamente indisponíveis para agendamentos 
        de clientes no seu sistema. Mais precisão, menos conflitos!
      </p>
      <button
      ...
      >
        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm5 14h-2v2h-2v-2h-2v-2h2v-2h2v2h2v2zM12 4c-4.41 0-8 3.59-8 8s3.59 8 8 8 8-3.59 8-8-3.59-8-8-8z"/></svg>
    ...
    <div className="space-y-8 bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.06)] border border-gray-200 dark:border-gray-700 min-h-[500px]">
      ...
      {loading && (
        <div className="text-center py-12 text-indigo-500">
          ...
          <p className="mt-2 text-sm">Carregando status...</p>
        </div>
      )}
      ...
      {!loading && isAuthorized && (
        <div className="p-4 rounded-3xl border border-gray-200 dark:border-gray-700 shadow-[0_8px_30px_rgba(0,0,0,0.06)] bg-white dark:bg-gray-800 overflow-hidden">
          ...
          <div className="mt-4 text-center">
            <button
                onClick={() => {
                  if (window.confirm("Deseja desconectar sua conta do Google Calendar? Isso irá liberar os horários bloqueados no seu sistema.")) {
                    // Endpoint para o backend revogar/desconectar o token
                    fetch(`${API_BASE_URL}/api/admin/calendar/disconnect/${slug}`, { method: 'POST', credentials: 'include' })
                      .then(res => {
                        if (res.ok) {
